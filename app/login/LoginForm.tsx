  ...
  return (
    <div className="mt-8">
      ...
      <form onSubmit={submit} className="mt-5 space-y-4">
        ...
        <button disabled={loading} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-teal-700 px-5 font-semibold text-white shadow-sm transition hover:bg-teal-600 disabled:cursor-wait disabled:bg-slate-300">{loading ? "Please wait…" : mode === "signup" ? "Create account" : "Sign in"}{!loading && <ArrowRight size={18} weight="bold" />}</button>
      </form>
    </div>
  );
}
